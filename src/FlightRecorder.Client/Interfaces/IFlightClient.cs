...
namespace FlightRecorder.Client.Interfaces
{
    public interface IFlightClient
    {
        ...
        Task<Flight> AddFlightAsync(string callsign, string number, string embarkation, string destination, long airlineId);
        Task<Flight> UpdateFlightAsync(long flightId, string callsign, string number, string embarkation, string destination, long airlineId);
