...
{
    [Authorize]
    public class ManufacturerStatisticsController : FlightRecorderControllerBase
    {
        ...
        public ManufacturerStatisticsController(
            ...
            ILogger<ManufacturerStatisticsController> logger) : base (renderer, logger)
        {
        ...
        public async Task<IActionResult> Index(ManufacturerStatisticsViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                List<ManufacturerStatistics> records = await _reportsClient.ManufacturerStatisticsAsync(start, end, page, _settings.SearchPageSize);
        ...
        {
            await _exportClient.ExportReport<ManufacturerStatistics>(model.From, model.To);
            return Ok();
        }
    }
}
